...
for apiAccess in apiDict:
    ...
    apiList.append(tweepy.API(authentication, wait_on_rate_limit=True))

def crawlTSV(given_tsv, countRequestNum, countClientNum, apiList):
    retrieveList = []
    for i, row in enumerate(given_tsv):
        ...
        if len(retrieveList) < 100:
            ...
            api = apiList[countClientNum % len(apiList)]
            ...
            if countRequestNum % 50 == 0:
                limits = api.rate_limit_status()
                timesRemain = limits['resources']['statuses']['/statuses/lookup']['remaining']
                # change api when touch request limit
                print("api to: ", countClientNum % len(apiList), "reading index: ", i)

                if timesRemain <=100:
                    countClientNum += 1
                    # update api
                    api = apiList[countClientNum % len(apiList)]
                    # sleep until first API alive
                    limits = api.rate_limit_status()
                    timesRemain = limits['resources']['statuses']['/statuses/lookup']['remaining']
                    if countClientNum % len(apiList) == 0 and timesRemain <= 100:
            ...
            countRequestNum += 1
            retrieveList = []
    return countRequestNum, countClientNum
...
for filename in os.listdir('covidTweetID/covidData')[25:]:  # . is cwd
    print(filename)
    tsv_file = open("covidTweetID/covidData/" + filename)
    read_tsv = csv.reader(tsv_file, delimiter="\t")
    # define request and client count
    countRequestNum, countClientNum = crawlTSV(read_tsv, countRequestNum, countClientNum, apiList)
